...
def GANs_train(dataset_configuration, train_configuration, results_path):
    ...
    with open("PretrainResults/best_models.txt", "r") as best_pretrain_models:
    ...
    pretrain_nontarget_samples = np.load(
        f"PretrainResults/pretrain_nontarget_samples_subject_{subject_index}.npy"
    )
    ...
    acc = np.full(number_of_trains, np.nan)
    kappa = np.full(number_of_trains, np.nan)
    ...
    for train_index in range(number_of_trains):
        run_number = train_index + 1

        generator_path = (
            file_path + '/' + GAN_models +
            '_generator_run_{}.keras'.format(run_number)
        )
        classification_resume_path = (
            file_path +
            '/classification_run_{}.weights.h5'.format(run_number)
        )
        metrics_path = (
            file_path +
            '/run_{}_metrics.npz'.format(run_number)
        )

        # A run is complete only when its generator,
        # classifier checkpoint and metrics all exist.
        if (
            os.path.exists(generator_path)
            and os.path.exists(classification_resume_path)
            and os.path.exists(metrics_path)
        ):
            saved_metrics = np.load(metrics_path)
            acc[train_index] = float(saved_metrics['acc'])
            kappa[train_index] = float(saved_metrics['kappa'])

            print(
                "Subject {} Run {} already complete "
                "(acc={:.4f}, kappa={:.4f}) - skipping.".format(
                    subject_index,
                    run_number,
                    acc[train_index],
                    kappa[train_index]
        ...
        classification_models_path = file_path + '/classification_run_{}.weights.h5'.format(train_index + 1)
        ...
        # Marks this run as successfully completed.
        np.savez(
            metrics_path,
            acc=acc[train_index],
            kappa=kappa[train_index]
        )
    ...
    best_run = np.nanargmax(acc)
    best_model_path = file_path + '/' + GAN_models + '_generator_run_{}.keras'.format(best_run + 1) + '\n'
    # Keep one line per subject in best_generator.txt.
    existing_best = []

    if os.path.exists(best_generator_path):
        with open(best_generator_path, "r") as f:
            existing_best = [line.rstrip("\n") for line in f]

    while len(existing_best) < subject_index:
        existing_best.append("")

    existing_best[subject_index - 1] = best_model_path.strip()

    with open(best_generator_path, "w") as f:
...
def run():
    ...
    train_configuration = {
        "batch_size": 64,
        "number_of_classification_training_epochs": 100,
        "classification_training_patience": 50,
        "learning_rate": 0.0001,
        "GPU_index": 0,
        "subject_index": 2,
        "number_of_trains": 10,
        "number_of_GAN_training_epochs": 5000,
        "augment_rate": 0.1,
        "GAN_models": "DiscoGAN",
