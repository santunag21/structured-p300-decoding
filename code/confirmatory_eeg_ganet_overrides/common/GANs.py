...
class GAN_discriminator_1:
    ...
    def build_model(self):
        # DiscoGAN generators operate on (channels, samples).
        # Add the singleton image-channel dimension internally for Conv2D.
        inputs = Input(shape=(self.NumChannels, self.NumSamples))
        inputs_4d = Reshape((self.NumChannels, self.NumSamples, 1))(inputs)

        convolution_block_1 = self.branch_convolution_block(inputs_4d, conv_dimension='temporal')
        convolution_block_2 = self.branch_convolution_block(inputs_4d, conv_dimension='spectral')
...
def DiscoGAN(OriginalData, OriginalLabel, SubjectIdx, SamplingFrequency, NumChannels=8, NumAugEpochs=1000,
    ...
    for epoch in range(NumAugEpochs):
        ...
        # Freeze discriminators while updating the generators
        discriminator_A.trainable = False
        discriminator_B.trainable = False
